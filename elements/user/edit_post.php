...
/**
 * Recieves form data and edits post
 * 
...
function handle_edit_post() {
    ...
    // Recieve thumbnail data from post
    $thumbnail_attachment_id = "";
    $thumbnail_vana = $_POST['thumbnail-old'];
    error_log("The old thumbnail " . $thumbnail_vana);
    $thumbnail = $_FILES['thumbnail'];
    

    if (!empty($thumbnail_vana)) {
        set_post_thumbnail($post_id, $thumbnail_vana);
    } else {
        ...
        error_log("Thumbnail attachment id " . $thumbnail_attachment_id);
        error_log("Thumbnail uploaded " . $post_id . " in edit." );
        // Set post thumbnail
        set_post_thumbnail($post_id, $thumbnail_attachment_id);
    };

    // Recieve gallery data from post
    $galerii = $_FILES['uploads'];
    $galerii_vana = $_POST['gallery-old'];
    // Make sure galerii_vana data is parsed properly
   
    if (!is_array($galerii_vana)) {
        $galerii_vana = [];
    }
    error_log("Previously uploaded gallery ids " . json_encode($galerii_vana));
    $galerii_attachment_ids = [];

    foreach ($galerii_vana as $id) {
        $galerii_attachment_ids[] = intval($id);
    }
    ...
    error_log("Galerii to be uploaded " . json_encode($galerii_attachment_ids));
